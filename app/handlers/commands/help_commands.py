...
async def cmd_help(message: Message):
    ...
    help_text += (
        "⭐ *Подписка и премиум:*\n"
        "• /sub — купить премиум-подписку за звёздочки или рубли\n"
    )
    if user_has_premium:
    ...
    if user_has_premium:
        ...
        help_text += (
            ...
            "• /triggers — список триггеров чата\n"
            "• /my_triggers — ваша статистика по триггерам\n"
            "Для доступа купите подписку через /sub\n\n"
        )
    ...
    if user_is_admin:
        help_text += (
            "👑 *Для администраторов:*\n"
            "• /admin_help — полный список админских команд\n"
            "• /admin_register — регистрация для уведомлений\n"
            "• /admin_feedback — управление обращениями\n\n"
        )

    if not user_has_premium and not user_is_admin:
        help_text += (
            "⭐ *Хотите больше возможностей?*\n"
            "Приобретите премиум-подписку через /sub!"
        )
    else:
        help_text += (
